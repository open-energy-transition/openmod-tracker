...
# ===== Tests for main page (⚡️_Tool_Repository_Metrics.py) =====
...
class TestGetStatsFunctions:
    ...
    @pytest.fixture(scope="class")
    def get_stats(self):
        """Load get_stats module (class-level fixture)."""
        inventory_dir = Path(__file__).parent.parent / "inventory"
        sys.path.insert(0, str(inventory_dir))
        # Temporarily replace "util" in sys.modules with inventory's util
        # so that get_stats.py imports the correct one
        original_util = sys.modules.get("util")
        inventory_util = load_module_from_file(inventory_dir / "util.py", "util")
        sys.modules["util"] = inventory_util
        module = load_module_from_file(inventory_dir / "get_stats.py", "get_stats")
        # Restore website util so other tests are unaffected
        if original_util is not None:
            sys.modules["util"] = original_util
        return module
    ...
    def test_get_score_card_returns_scorecard_on_success(
        self, get_stats, sample_scorecard, monkeypatch
    ) -> None:
        ...
        assert result == sample_scorecard
