...
from streamlit import markdown, set_page_config, subheader, title, write


def update_esm_analysis(url_api: str):
    """Get ESM tool repo data from ecosyste.ms and create streamlit page to view it.

    Args:
        url_api (str): ecosyste.ms API url.
    """
    # define variables
    names = []
    urls = []
    ...
    for i in range(len(all_data)):
        ...
        for package_manager in range(len(json_data["packages"])):
            if json_data["packages"][package_manager]["downloads"]:
                if (
                    json_data["packages"][package_manager]["downloads_period"]
                    == "last-month"
                ):
                    package_downloads += json_data["packages"][package_manager][
                        "downloads"
                    ]

            if json_data["packages"][package_manager]["dependent_repos_count"]:
                dependent_repos_count += json_data["packages"][package_manager][
                    "dependent_repos_count"
                ]

            if (
                latest_release_published_at is None
                or latest_release_published_at
                < json_data["packages"][package_manager]["latest_release_published_at"]
            ):
                latest_release_published_at = json_data["packages"][package_manager][
        ...
        develop_distr_scores.append(f"{json_data['commits']['dds']:.3f}")
        past_year_issues_counts.append(
            json_data["issues_stats"]["past_year_issues_count"]
        )
        creates.append(
            datetime.strptime(
                json_data["repository"]["created_at"], "%Y-%m-%dT%H:%M:%S.%fZ"
            ).strftime("%Y/%m")
        )
        updates.append(
            datetime.strptime(
                latest_release_published_at, "%Y-%m-%dT%H:%M:%S.%fZ"
            ).strftime("%Y/%m")
        )
    ...
    df.drop(
        columns=["Category", "Sub Category", "Language", "License", "Citations"],
        axis=1,
        errors="ignore",
        inplace=True,
    )
    ...
    write(
        "This analysis is available at OET's GitHub repository <a href='https://github.com/open-energy-transition/open-esm-analysis/' target='_new'>open-esm-analysis</a>.",
    ...
    write("")
    markdown(
        "**Table 1: Selected Open-Source ESM Tools - Key Data Indicators** (Data: ecosystem.ms; Last Update: "
    ...
    markdown(
        "**Would you find a real-time OS tool insight platform useful?** Share your thoughts and suggestions in the comments or the <a href='https://github.com/open-energy-transition/open-esm-analysis/issues'>issues tracker</a>!",
        unsafe_allow_html=True,
    )
    write("")
